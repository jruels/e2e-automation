...
Get the external IP address and access our application:
```
export SERVICE_IP=$(kubectl get svc helm-demo-app -o jsonpath='{.status.loadBalancer.ingress[0].ip}')
...
We also need to change the Chart version number from `0.0.1` to `0.2.0` in the `Chart.yaml` to show we have made a significant change. 
